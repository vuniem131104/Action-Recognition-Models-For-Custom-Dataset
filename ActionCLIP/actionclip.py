...
parser.add_argument('--tracking_file', type=str, help='The tracking file.')
...
def main():
    ...
    if args.tracking_file is not None:
        ...
        with open(os.path.join('tracking_file', args.tracking_file), 'r') as f:
            for line in f:
                cur_frame, person_id, x1, y1, x2, y2 = line.strip().split(',')[:6]
                cur_frame = int(float(cur_frame))
                if cur_frame not in datas:
                    datas[cur_frame] = []
                datas[cur_frame].append([int(person_id), int(float(x1)), int(float(y1)), int(float(x2)), int(float(y2))])
        print("Loaded data")
            ...
            while ret:
                ...
                if frame_cnt in datas:
                    for data in datas[frame_cnt]:
                        ...
                        # print(person_id, x11, y11, x22, y22)
                        x1 = max(int(x11 - 40), 0)
                        y1 = max(int(y11 - 40), 0)
                        x2 = min(int(x22 + 40), w)
                        y2 = min(int(y22 + 40), h)
        ...
        with open(f'result_txt/{video_path}_{people_detection_threshold}.txt', 'w') as file:
            while ret:
                annotated_frame = frame.copy()
                results = detector.predict(frame, device=device)[0]
                detections = sv.Detections.from_ultralytics(results)
                detections = detections[(detections.class_id == 0) & (detections.confidence > people_detection_threshold)]
                detections = tracker.update_with_detections(detections)

                for person_id, xyxy in zip(detections.tracker_id, detections.xyxy):
                    ...
                    annotated_frame = cv2.rectangle(annotated_frame, (x11, y11), (x22, y22),
                                                    (colors[person_id % len(colors)]), 2)
                    annotated_frame = cv2.putText(annotated_frame, action, (x11 + 10, y11 + 30), cv2.FONT_HERSHEY_SIMPLEX,
                                                  0.9, (0, 0, 255), 2)
                    file.write(text)
